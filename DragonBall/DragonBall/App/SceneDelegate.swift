...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let rootViewController = storyboard.instantiateViewController(withIdentifier: SplashViewController.identifier) as? SplashViewController

        rootViewController?.viewModel = SplashViewModel(
            apiProvider: DragonBallDataProvider(),
            secureDataProvider: KeychainDataProvider()
        )
